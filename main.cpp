...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (window_alpha > 0.0f) 
        {
            ...
            if (appConfig.show_cpu_usage) ImGui::TextColored(text_color, "CPU: %.2f%%", systemMonitor.getCpuUsage());
            if (appConfig.show_cpu_temp) ImGui::TextColored(text_color, "CPU Temp: %d C", systemMonitor.getCpuTemperature());
            if (appConfig.show_memory_stats) ImGui::TextColored(text_color, "Mem: %lld MB / %lld MB (%.2f%%)",
            ...
            if (appConfig.show_net_down) ImGui::TextColored(text_color, "Net Down: %.2f KB/s", systemMonitor.getDownloadSpeed() / 1024.0);
            if (appConfig.show_net_up) ImGui::TextColored(text_color, "Net Up: %.2f KB/s", systemMonitor.getUploadSpeed() / 1024.0);
            if (appConfig.show_ping) ImGui::TextColored(text_color, "Ping: %.2f ms", systemMonitor.getPingLatency());
            if (appConfig.show_gpu_usage) ImGui::TextColored(text_color, "GPU Usage: %.2f%%", systemMonitor.getGpuUsage());
            if (appConfig.show_gpu_mem) ImGui::TextColored(text_color, "GPU Mem: %lld MB / %lld MB", systemMonitor.getGpuMemoryUsed(), systemMonitor.getGpuMemoryTotal());
            if (appConfig.show_gpu_temp) ImGui::TextColored(text_color, "GPU Temp: %d C", systemMonitor.getGpuTemperature());
            if (appConfig.show_fps) ImGui::TextColored(text_color, "FPS: %.2f", systemMonitor.getFps());
        ...
        if (debug_mode && !config_mode) {
            ...
            ImGui::SetNextWindowBgAlpha(window_alpha); 
            ImGui::Begin("Debug Stats", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoBackground);

            if (appConfig.show_app_cpu) ImGui::TextColored(text_color, "App CPU: %.2f%%", systemMonitor.getProcessCpuUsage());
            if (appConfig.show_app_mem) ImGui::TextColored(text_color, "App Mem: %lld KB", systemMonitor.getProcessMemoryUsage());
        ...
        if (config_mode && show_config_window) {
            ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x / 2, io.DisplaySize.y / 2), ImGuiCond_Always, ImVec2(0.5, 0.5));
            ImGui::Begin("Configuration", &show_config_window, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);

            if (ImGui::CollapsingHeader("General Display")) {
                ImGui::Checkbox("Enable Fading Colors", &appConfig.enable_fading_colors);
                ImGui::SliderFloat("Hue Speed", &appConfig.hue_speed, 0.01f, 0.5f, "%.2f");
